...
var DiffExtension_exports = {};
__export(DiffExtension_exports, {
  AddedText: () => AddedText,
  DiffExtension: () => DiffExtension,
  RemovedText: () => RemovedText,
  applyDiffToContent: () => applyDiffToContent,
  default: () => DiffExtension_default
});
module.exports = __toCommonJS(DiffExtension_exports);
...
var Selection = class {
  ...
  */
  replace(tr, content = import_prosemirror_model2.Slice.empty) {
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      tr.replaceRange(mapping.map($from.pos), mapping.map($to.pos), i ? import_prosemirror_model2.Slice.empty : content);
      if (i == 0)
...
var TextSelection = class _TextSelection extends Selection {
  ...
  */
  ...
  replace(tr, content = import_prosemirror_model2.Slice.empty) {
    ...
    if (content == import_prosemirror_model2.Slice.empty) {
...
var NodeSelection = class _NodeSelection extends Selection {
  ...
  */
  ...
  content() {
    return new import_prosemirror_model2.Slice(import_prosemirror_model2.Fragment.from(this.node), 0, 0);
  }
  eq(other) {
...
// src/components/chatInput/DiffExtension.ts
var AddedText = import_core.Mark.create({
  name: "added",
  addOptions() {
    return {
      HTMLAttributes: {
  ...
  addCommands() {
    return {
      setAddedText: () => ({ commands }) => {
        return commands.setMark(this.name);
      },
      toggleAddedText: () => ({ commands }) => {
        return commands.toggleMark(this.name);
      },
      unsetAddedText: () => ({ commands }) => {
...
var RemovedText = import_core.Mark.create({
  name: "removed",
  addOptions() {
    return {
      HTMLAttributes: {
  ...
  addCommands() {
    return {
      setRemovedText: () => ({ commands }) => {
        return commands.setMark(this.name);
      },
      toggleRemovedText: () => ({ commands }) => {
        return commands.toggleMark(this.name);
      },
      unsetRemovedText: () => ({ commands }) => {
        return commands.unsetMark(this.name);
      }
    };
  },
...
var DiffExtension = import_core.Mark.create({
  name: "diff",
  addExtensions() {
  ...
  addCommands() {
    return {
      ...
      markSelectionAsDiff: (type) => ({ commands }) => {
        if (type === "added") {
...
function applyDiffToContent(editor, oldText, newText) {
  editor.commands.replaceWithDiff({ oldText, newText });
}
var DiffExtension_default = DiffExtension;
...
//# sourceMappingURL=DiffExtension.js.map