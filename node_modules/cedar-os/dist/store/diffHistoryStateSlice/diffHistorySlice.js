...
var diffHistorySlice_exports = {};
__export(diffHistorySlice_exports, {
  createDiffHistorySlice: () => createDiffHistorySlice
});
module.exports = __toCommonJS(diffHistorySlice_exports);
...
function setValueAtPathForDiff(obj, path, value) {
  ...
  const pathParts = path.startsWith("/") ? path.slice(1).split("/") : path.split("/");
  const result = (0, import_lodash.cloneDeep)(obj);
  let current = result;
...
function areStatesEquivalent(oldState, newState) {
  return (0, import_lodash.isEqual)(oldState, newState);
}
...
var createDiffHistorySlice = (set, get) => ({
  ...
  registerDiffState: (config) => {
    ...
    if (!existingDiffState) {
      ...
      if (!(0, import_lodash.isEqual)(currentNewState, value) && !(0, import_lodash.isEqual)(currentComputedState, value)) {
  ...
  setDiffState: (key, diffHistoryState) => {
    ...
    if (newState !== void 0) {
      ...
      if (!registeredState) {
        ...
        if (!(0, import_lodash.isEqual)(currentValue, newState)) {
          set(
            (state) => ({
              registeredStates: {
  ...
  newDiffState: (key, newState, isDiffChange) => {
    ...
    const patches = (0, import_fast_json_patch.compare)(oldStateForDiff, newState);
    ...
    if (registeredState) {
      const currentValue = registeredState.value;
      if (!(0, import_lodash.isEqual)(currentValue, computedStateValue)) {
        set(
          (state) => ({
            registeredStates: {
              ...state.registeredStates,
  ...
  applyPatchesToDiffState: (key, patches, isDiffChange) => {
    ...
    const updatedHistory = [...history, originalDiffState];
    const currentNewState = (0, import_lodash.cloneDeep)(originalDiffState.newState);
    const patchResult = (0, import_fast_json_patch.applyPatch)(
      currentNewState,
      patches,
      false,
    ...
    const diffPatches = (0, import_fast_json_patch.compare)(
      oldStateForDiff,
      patchResult
    );
...
//# sourceMappingURL=diffHistorySlice.js.map