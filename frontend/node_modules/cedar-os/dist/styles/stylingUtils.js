...
var stylingUtils_exports = {};
__export(stylingUtils_exports, {
  cn: () => cn,
  createBorderColor: () => createBorderColor,
  desaturateColor: () => desaturateColor,
  getLightenedColor: () => getLightenedColor,
  getShadedColor: () => getShadedColor,
  getTextColorForBackground: () => getTextColorForBackground,
  getThemeFromBackground: () => getThemeFromBackground,
  hexToRgb: () => hexToRgb,
...
module.exports = __toCommonJS(stylingUtils_exports);
...
var getShadedColor = (hex, shade) => {
  ...
  return `rgb(${Math.max(0, r2 - shade)}, ${Math.max(0, g - shade)}, ${Math.max(
    0,
    b - shade
  )})`;
};
var getLightenedColor = (hex, lighten) => {
  const { r: r2, g, b } = hexToRgb(hex);
  return `rgb(${Math.min(255, r2 + lighten)}, ${Math.min(
    255,
    g + lighten
...
function isLightColor(color) {
  ...
  const r2 = parseInt(fullHex.substring(0, 2), 16);
...
var getThemeFromBackground = (backgroundColor) => {
  const hex = backgroundColor.replace("#", "");
  const r2 = parseInt(hex.substring(0, 2), 16);
  const g = parseInt(hex.substring(2, 4), 16);
  const b = parseInt(hex.substring(4, 6), 16);
